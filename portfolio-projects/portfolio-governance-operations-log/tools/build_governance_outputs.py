...
OUT_MD = ROOT / "sample-outputs" / "markdown"
OUT_HTML = ROOT / "sample-outputs" / "html"
OUT = ROOT / "sample-outputs"
...
def main() -> int:
    ...
    for filename, content in outputs.items():
        md_path = OUT_MD / filename
        write(md_path, content)
        html_name = filename.replace("_", "-").replace(".md", ".html")
        write(OUT_HTML / html_name, simple_markdown_to_html(filename.replace("_", " ").replace(".md", "").title(), content))

    write_findings(classified)
    print(f"Generated {len(outputs)} Markdown outputs and {len(outputs)} HTML outputs.")
